...
class BookingsController extends Controller
{
    ...
    public function edit($id)
    {
        ...
        toastr()->success('Booking edited successfully.');

        return view('admin.bookings.edit', compact('booking', 'customers', 'rooms'));
    }

    public function add_days(Request $request, $id)
    {
        $booking = Booking::findOrFail($id);

        $org_time_to = $booking->time_to;

        $this->validate($request, [
            'time_to' => 'required|date_format:'.config('app.date_format'). ' H:i',
            'add_amount' => 'required',
            'add_payment_mode' => 'required'
        ]);

        Booking::whereId($id)->update([
            'time_to' => $request->time_to,
            'org_time_to' => $org_time_to,
            'add_amount' => $request->add_amount,
            'add_payment_mode' => $request->add_payment_mode
        ]);
    ...
    public function show($id)
    {
        ...
        $date1 = Carbon::parse($booking->time_to)->format('Y-m-d');
        $date2 = Carbon::parse($booking->org_time_to)->format('Y-m-d');
    ...
    public function perma_del($id)
    {
        ...
        Room::whereId($booking->room_id)->update([
            'booked' => false
        ]);
